...
|---|---|---|
| Single video | `platform_data.tiktok.media_type: "video"` or omit (default) | One `media` entry required. Accepted formats: MP4, MOV, WebM. |
...
| `media_type` | string | No | `"video"` (default) or `"photo"`. Case-insensitive, so TikTok's own spelling `"PHOTO"` is accepted too. A value that names neither post type is rejected with a `400` on the `media_type` field, by `POST /v1/posts/validate` as well as by `POST /v1/posts`, rather than being published as a video. |
...
Sending fewer than 2 or more than 35 `media` entries when `media_type` is `"photo"` returns a `400` validation error immediately. Single-image photo posts are not supported by TikTok.
