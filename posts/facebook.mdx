...
| Single photo | One image entry in `media` | Routes to `/{page-id}/photos`; `platform_data` is ignored |
| Single video | One video entry in `media` (`.mp4`, `.mov`, `.avi`, `.mkv`, `.webm`) | Routes to `/{page-id}/videos`; only the `title` platform_data field is forwarded |
| Reel | One vertical video entry in `media` plus `content_type: "reel"` in the target's `platform_data` | Routes to the Reels API (`/{page-id}/video_reels`). Unlike standard videos, `platform_data` IS honored (`content_type`, `share_to_feed`). |
...
| Multi-photo album | 2 to 10 image entries in `media` | Each photo is uploaded with `published=false`, then a single `/{page-id}/feed` call attaches all `media_fbid` values via `attached_media[]`. The target's `platform_data` fields (`link`, CTA, age targeting) are forwarded to the feed call. |
...
Provide a single image entry in `media`. The `alt_text` platform_data field is forwarded as `alt_text_custom` on the photo; all other `platform_data` keys are ignored for single-photo posts.
...
Provide 2 to 10 image entries in `media` to publish a multi-photo album. SocialAPI handles the two-step Graph API flow internally: each photo is uploaded with `published=false` to obtain a `media_fbid`, then a single `/{page-id}/feed` call attaches all photo IDs via `attached_media[]` and sets the caption.
...
Provide a single video entry in `media`. SocialAPI detects the video extension and routes to `/{page-id}/videos`. The `title` platform_data field is forwarded as the video title; all other `platform_data` keys are ignored for single-video posts.
...
- Reels are included, and report a different set of counts than feed posts. A Reel is published through the Reels API and does not appear in the Page feed at all, so SocialAPI reads it directly by its video ID. A Reel returns `likes`, `comments`, and a view count under `metrics.extra.views`. `shares` stays `0` for a Reel: a share count for Reels exists only on the Graph API's video insights edge, which requires the `read_insights` permission that SocialAPI does not currently request, and it is reported there combined with comments rather than as a separate figure. We leave the field at `0` rather than report a number we cannot read.
