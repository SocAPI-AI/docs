...
| `brand_id` | top-level | optional | Attach the account to an existing brand. If omitted, your unused "Default" brand is reused when available; otherwise a new brand is auto-created. |
...
### Pending connections: choosing among multiple candidates

Some platforms cannot resolve a single account from an OAuth grant alone, so the connect flow pauses and asks you to choose before an account is created. Google takes this path only when a login manages more than one Business Profile. Facebook takes it on every connect, because one Facebook login can serve many brands and Pages must be assigned to a brand explicitly.

When this happens, your `redirect_uri` (or the exchange response, in the proxy flow) receives `status: selection_required` and a `connection_id` instead of `status: success`. No account exists yet. Finish the connection with `POST /v1/accounts/pending/{connection_id}/select`, shaped differently per platform as shown below.

### Google: connecting a login with multiple Business Profiles

A Google login with a single Profile connects like any other platform, in a single request, with no extra step.
...
**Connecting a second Profile later.** The pending connection expires, but the login itself is kept. Once a Google login has been connected, its other Business Profiles can be attributed to your brands at any time without a new authorization. Both endpoints work from the login (credential) id, returned as `login_id` on the pending connection and on every account that login backs.

`GET /v1/platforms/google/logins/{loginId}/locations` lists everything the stored login can manage, read live from Google and merged with what you have already attributed. A Profile created in Google Business Profile after that authorization appears here with no re-authorization. `login_email` identifies the Google account itself; it is empty for a login authorized before that was captured.

```bash
curl https://api.social-api.ai/v1/platforms/google/logins/cred_01J.../locations   -H "Authorization: Bearer $SOCAPI_KEY"
```
  ...
  "locations": [
    {
      ...
      "assigned_brand_id": "b7f9...",
      "assigned_brand_name": "Downtown",
      "assignable": false
    },
    {
      "platform_account_id": "accounts/123/locations/789",
      "name": "Joe's Pizza (Uptown)",
      "assignable": true
    }
  ],
  "count": 2
...
`POST /v1/platforms/google/logins/{loginId}/locations/{locationId}/assign` attributes one of them to a brand. A location id is a compound resource name containing slashes, so percent-encode it in the path:

```bash
curl -X POST   "https://api.social-api.ai/v1/platforms/google/logins/cred_01J.../locations/accounts%2F123%2Flocations%2F789/assign"   -H "Authorization: Bearer $SOCAPI_KEY"   -H "Content-Type: application/json"   -d '{"brand_id": "b7f9..."}'
```

<Note>
  Each Business Profile belongs to exactly one brand, and each brand holds at most one Google Business Profile.

  Assigning to a brand that already has a Profile returns `409` (`platform.google.brand_has_account`). Assigning a location another brand already holds returns `400` (`validation.location_already_assigned`), as does a location the login can no longer manage (`validation.location_not_granted`). An unknown login or brand returns `404`.

  A successful assign returns `201` with `account_id`, `brand_id`, and `platform_account_id`. Assigning a location to the brand that already holds it is idempotent: it returns the existing account rather than an error.
</Note>

### Facebook: choosing which Pages to connect
...
**Server-callback flow.** Your `redirect_uri` receives `status=selection_required` and a `connection_id`, the same shape as Google:
...
**Proxy flow.** `POST /v1/oauth/exchange` returns the pending connection in the body, with Facebook-specific fields alongside the shared `status` and `connection_id`:
...
```json
{
  ...
  "pages": [
    ...
    },
    {
      ...
      "name": "Acme Cafe",
      "assigned_brand_id": "b_01HZ9X3Q4R5M6N7P8V2K0W1J",
      "assignable": false
    }
  ],
  "lost_access": []
...
| Field | Meaning |
|---|---|
| `login_id` | Identifies the shared Facebook credential behind this connection. Use it later with the [Facebook logins endpoints](/connectors/facebook#managing-pages-after-connecting) to list or assign Pages without a new OAuth round trip. |
| `pages[].platform_page_id` | The Facebook Page ID. |
| `pages[].name` | The Page's display name. |
| `pages[].assigned_brand_id` | Empty when the Page is unassigned and available. Set to an existing brand ID when one of your other brands already owns the Page, in which case `assignable` is `false`. |
| `pages[].assignable` | Whether this Page can be assigned to the brand you are connecting into right now. |
| `lost_access[]` | Pages this authorization revoked access to, app-wide. Empty on a first connect; see below. |

**Resolving the selection.** As with Google, `GET /v1/accounts/pending/{connection_id}` re-fetches the candidate list, and `POST /v1/accounts/pending/{connection_id}/select` completes the connection, but the body carries `page_ids` instead of a single `platform_account_id`:

```bash
curl -X POST https://api.social-api.ai/v1/accounts/pending/pc_4a8f2c1e9b3d7f06a5c2e8b4d1f3a7e2/select \
  -H "Authorization: Bearer $SOCAPI_KEY" \
  -H "Content-Type: application/json" \
  -d '{"page_ids": ["17841400000000000"]}'
```

You do not have to assign every candidate Page at connect time. Pages you leave out stay unassigned and can be picked up later, from this brand or another one you own, via `GET /v1/platforms/facebook/logins/{login_id}/pages` and `POST /v1/platforms/facebook/logins/{login_id}/pages/{page_id}/assign`. See [Facebook connector: managing Pages after connecting](/connectors/facebook#managing-pages-after-connecting).

**Lost access.** Facebook grants access per app, not per brand. When a user re-authenticates and unchecks a Page they had previously granted, that Page's access is revoked everywhere, including on brands other than the one being reconnected. `lost_access[]` names every Page this happened to, along with the brand that was using it:
...
"lost_access": [
  {
    ...
    "brand_name": "Acme Cafe LLC"
  }
]
```

Show the affected user this warning, filling in the Page and brand names from the entry:

> Reconnecting removed access to {page}, used by {brand}. To restore it, reconnect Facebook and choose Edit previous settings, keeping {page} checked.

The brand's assignment to a lost Page is preserved, not deleted. The next time the same login is re-granted access to that Page (by keeping it checked on Facebook's consent screen during a later reconnect), it reactivates in place, on the same brand, with no further steps.

<Note>
  A Facebook pending connection is single-use and expires 30 minutes after it is created, the same rule as Google. Selecting a `platform_page_id` you have no access to, or that another brand already owns, returns `400`.
</Note>
