...
| `http.<status>` | Fiber framework HTTP error (for example, `http.404` for an unknown route). Not returned by application logic. |
...
| 403 | `auth.invalid_secret` | Provided client secret does not match |
...
| 403 | `auth.insufficient_scope` | A [scoped API key](/guides/scoped-keys) lacks the required scope, or a restricted key called an admin operation. `meta.required_scope` names the scope when applicable |
| 403 | `auth.state_mismatch` | OAuth callback `state` does not match the issued value |
...
| 401 | `auth.oauth.user_missing` | OAuth grant references a user that does not exist |
...
| 400 | `account.no_active_page` | Account has no active Page (Facebook, LinkedIn) selected |
| 400 | `account.page_inactive` | The requested Page is currently disabled |
| 400 | `account.pages_not_supported` | Pages do not apply to this platform |
| 409 | `account.different_login` | Brand's Facebook account is tied to a different login (shared credential); disconnect it before connecting a new one |
...
| 400 | `validation.scope_invalid` | A [key](/guides/scoped-keys) was created or edited with an unknown scope; the message names it |
| 400 | `validation.brand_invalid` | A key's `brand_ids` references a brand that does not exist or that you do not own |
...
Both share the `resource.conflict` code. Note that `GET /v1/invites` and `GET /v1/accounts` query different objects, so an empty invite list does not guarantee that a create will succeed: the brand may already have a connected account for that platform. Check both before treating a create as safe.
...
| 403 | `billing.profile_units_limit` | Profile unit count would exceed your plan's allowance (creating a new brand) |
...
| 402 | `billing.past_due` | Subscription is past due after two failed payment attempts. See below |

Limit responses include `error.meta.plan` and `error.meta.max` so clients can prompt the user to upgrade.

### Past-due accounts

When a subscription's payment fails twice (the initial charge plus one automatic
retry), API access is paused. Every `/v1` endpoint returns `402` with code
`billing.past_due` until the invoice is paid.

Three things stay reachable so the account can be recovered:

- `GET /v1/users/me` and `PATCH /v1/users/me`
- every `/v1/billing/*` endpoint
- inbound platform webhooks, which are unaffected

`GET /v1/users/me` reports the state in two fields: `billing_blocked` (boolean)
and `billing_blocked_at` (when the block began, or null).

**Outbound webhooks are not delivered while an account is past due.** Events that
occur during this window are dropped, not queued, and are not replayed after
payment. Ingestion is unaffected, so the underlying data is still recorded and
remains queryable through `GET /v1/events` once access is restored.

Access is restored automatically within seconds of a successful payment. There is
no endpoint to call and no support ticket to open: paying the invoice, whether
through the dashboard's Stripe portal or an automatic retry, lifts the block.
...
| 403 | `platform.instagram.permission_denied`, `platform.facebook.permission_denied`, `platform.whatsapp.permission_denied` | OAuth scopes do not include the action |
| 400 | `platform.instagram.window_closed`, `platform.facebook.window_closed` | The 24-hour messaging window has closed; the user must message you first to reopen it. After the first refusal SocialAPI answers further sends on that thread itself, adding `blocked_locally` to the error meta (see [Inbox](/guides/inbox)) |
| 400 | `platform.instagram.private_reply_ineligible`, `platform.facebook.private_reply_ineligible` | The comment cannot receive a private reply: the 7-day window has expired, or the comment already received one |
...
| 403 | `platform.facebook.page_cap` | Brand already has the maximum of 5 Facebook Pages; this is a hard cap, not billed |
| 403 | `platform.youtube.account_suspended` | YouTube has suspended or closed the account, or its channel. The stored token is still valid, so reconnecting does not restore access: the suspension has to be resolved with YouTube directly. SocialAPI stops background sync for the account and sets `reconnect_reason` to `youtube_account_suspended` |
...
| HTTP | Code | When it occurs |
|---|---|---|
| 400 | `whatsapp.window_closed` | 24-hour customer service window is closed; send an approved template instead |
...
| 403 | `whatsapp.user_opted_out` | Recipient has opted out of marketing messages |
| 400 | `whatsapp.undeliverable` | Message could not be delivered (cold number, region block, etc.) |
...
| 404 | `whatsapp.no_account` | No WhatsApp account is connected for this request |
...
Zalo uses the generic `platform.zalo.*` namespace (see the `platform.<name>.*` row above), not a dedicated top-level namespace.

| HTTP | Code | When it occurs |
|---|---|---|
| 422 | `platform.zalo.not_deliverable` | Outside the customer service messaging window, or the recipient is not eligible |
| 401 | `platform.zalo.auth` | Access token expired or revoked, or the Official Account is no longer authorized |
| 202 | `platform.zalo.article_processing` | Article creation is still processing on Zalo after the verification poll budget is exhausted; the article may still publish, so check before retrying |
| 429 | `platform.zalo.rate_limit` | Zalo per-app or per-OA quota exhausted |
| 400 | `validation.platform_param` | Zalo rejected a parameter |
| 404 | `resource.not_found` | The referenced Zalo object or account does not exist |
| 502 | `platform.zalo.e<N>` | Zalo returned an error code SocialAPI does not map explicitly, `<N>` is the absolute value of Zalo's numeric code (Zalo `-105` becomes `platform.zalo.e105`) |
| 502 | `platform.zalo.api_error` | Transport or malformed-response failure reaching Zalo, with no Zalo error envelope to read a code from |

`platform.zalo.not_deliverable` is the one you're most likely to hit: Zalo only allows a customer service message within 7 days of the user's last interaction with your Official Account (free within 48 hours), and rejects sends outside that window with a 422 instead of queuing them.

## Bluesky

Bluesky uses the generic `platform.bluesky.*` namespace (see the `platform.<name>.*` row above), not a dedicated top-level namespace.

| Status | Code | Meaning |
|---|---|---|
| 401 | `platform.bluesky.auth_factor_required` | The account has email two-factor authentication enabled. Bluesky emailed a confirmation code; retry the connect request with it as `metadata.auth_factor_token` |
| 403 | `platform.bluesky.dm_scope_missing` | The app password was created without direct message access. DM access cannot be added to an existing app password: create a new one with the option enabled and reconnect. Posts, comments, and mentions keep working |
| 429 | `platform.bluesky.rate_limit` | Bluesky quota exhausted. Sign-in is limited to 30 requests per 5 minutes per account |

`platform.bluesky.dm_scope_missing` only affects the DM endpoints, so treat it as a per-capability failure rather than a dead connection.
