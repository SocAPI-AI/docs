...
### When the messaging window is closed

Instagram and Facebook only let you message someone inside a window that their last message to you opened. Once it closes, a send returns HTTP `400` with `platform.instagram.window_closed` or `platform.facebook.window_closed`.

After the platform refuses a thread for this reason, SocialAPI remembers it and answers further sends on that thread itself, with the same status and code plus `"blocked_locally": true` in the error meta. Nothing is sent to the platform, so the call returns immediately, costs no interaction credit, and does not count against your platform rate limits. The block lifts as soon as the person messages you again, on the webhook or the next message sync, and in any case after 24 hours, so a webhook that never arrives cannot leave a thread permanently unreachable.

A send that carries a `message_tag` is never blocked this way, since tags are how a message legitimately reaches someone outside the standard window.
...
What `GET /mentions` returns differs by platform, and on Instagram it is narrower than the name suggests. It returns media your account was tagged in, which is the only mention-like collection Meta exposes as a list. @mentions written inside a post caption or inside a comment are not included: Meta provides no endpoint to poll them on either Instagram connection flow, and they reach applications only as webhooks. Those webhooks are not currently delivered for Instagram either (see [`mention.received`](/guides/webhooks)), so an @mention of your account in a comment on someone else's post will not appear through this endpoint today.
...
|----------|----------|----------|
| Instagram | Tagged media only | Yes |
| Facebook | Yes | Yes |
| X / Twitter | Yes | No |
| All others | No | No |
