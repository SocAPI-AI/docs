...
- **Account suspension.** If YouTube suspends or closes the account or its channel, every call returns `403` with `code: "platform.youtube.account_suspended"`. Unlike the case above, the token is still valid and reconnecting changes nothing: the suspension has to be resolved with YouTube. SocialAPI stops syncing the account and flags it with `reconnect_reason` `youtube_account_suspended`, and emits an `account.disconnected` event. Reconnect once the account is reinstated to resume syncing.
